...
const PATTERNS: Record<string, Record<number, PatternDefinition>> = {
...
const MARKS: Record<number, Record<'done' | 'error', number[]>> = {
  3: { done: [2, 3, 5, 7], error: [0, 2, 4, 6, 8] },
  4: { done: [7, 8, 10, 13], error: [0, 3, 5, 6, 9, 10, 12, 15] }
};

const resolvePattern = (pattern: string | PatternDefinition, grid: number): PatternDefinition => {
  ...
  const max = validCells.length > 0 ? Math.max(...validCells) : 0;
...
export type LatticeLoaderPattern =
  | 'arrow'
  | 'dots'
  | 'ripple'
  | 'spiral'
...
export default function LatticeLoader({
  ...
  const marks = MARKS[n];
  const d = step * (pat.scale ?? 1);
  const cycle = Math.round((pat.loop ?? 8) * d);
