...
const CASCADE_LETTER_VARIANTS: Variants = {
  ...
  exit: (delay: number = 0) => ({
    ...
    y: '-105%',
    filter: ROLL_BLUR,
    transition: { duration: 0.16, ease: EASE_OUT as any, delay: delay * 0.5 },
  }),
};
...
const ICON_VARIANTS: Variants = {
  ...
  exit: {
    ...
    scale: 0.7,
    filter: ROLL_BLUR,
    transition: { duration: 0.16, ease: EASE_OUT as any },
  },
};
...
export const StatefulButton = forwardRef<HTMLButtonElement, StatefulButtonProps>(function StatefulButton(
  ...
  return (
    <Button ref={ref} disabled={disabled || isBusy} aria-busy={isBusy} whileHover={undefined} {...rest}>
      <span
        aria-live="polite"
        className="relative inline-flex items-center justify-center overflow-hidden gap-1.5"
      >
        <AnimatePresence initial={false}>
          {state === 'loading' ? (
