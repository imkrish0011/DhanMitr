...
/**
 * Unified theme hook resolving the active theme and setter across
 * DhanMitr's ThemeContext and next-themes.
 */
function useUnifiedTheme() {
  try {
    const appCtx = useAppTheme();
    if (appCtx && typeof appCtx.setTheme === "function") {
      return {
        theme: appCtx.theme,
        resolvedTheme: appCtx.resolvedTheme || appCtx.theme,
        setTheme: (t: string) => appCtx.setTheme(t as "light" | "dark"),
      };
    }
  } catch {}

  try {
    const nextCtx = useNextTheme();
    if (nextCtx && typeof nextCtx.setTheme === "function") {
      return {
        theme: nextCtx.theme,
        resolvedTheme: nextCtx.resolvedTheme || nextCtx.theme,
        setTheme: nextCtx.setTheme,
      };
    }
  } catch {}
...
export function useThemeToggle({
  ...
  const { setTheme, resolvedTheme } = useUnifiedTheme();
  ...
  const toggle = () => {
    ...
    ).startViewTransition(applyThemeChange);
  ...
  return (
    ...
    >
      {mounted ? (
        ...
        >
          {isDark ? (
            <Sun className={cn(iconClassName || "w-4 h-4", "text-amber-400")} />
          ) : (
            <Moon className={cn(iconClassName || "w-4 h-4", "text-indigo-600 dark:text-indigo-400")} />
          )}
        </ActionSwapIcon>
      ) : (
        <span className={cn(iconClassName || "w-4 h-4", "inline-block")} aria-hidden="true" />
      )}
    </button>
  );
}

// Re-export useTheme for drop-in compatibility
export { useUnifiedTheme as useTheme };
